...
import { NominationCategory, NominationStatus, CATEGORY_LABELS, STATUS_LABELS } from "@/lib/types";
  ...
  return (
    <span
      className={cn(
        ...
        variant === "default" && "bg-white/5 text-foreground-muted border border-white/10",
        ...
        variant === "gold" && "gold-shimmer text-white border border-amber-400/30",
        className
      )}
    >
      {children}
...
export function CategoryBadge({ category }: { category: NominationCategory }) {
  const colors: Record<NominationCategory, string> = {
    ...
    "customer-excellence": "bg-emerald-500/10 text-emerald-400 border-emerald-500/20",
    "above-and-beyond": "bg-amber-500/10 text-amber-400 border-amber-500/20",
  };

  return (
    <span className={cn("inline-flex items-center gap-1 px-2 py-0.5 rounded-[4px] text-xs font-medium border", colors[category])}>
